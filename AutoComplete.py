...
def get_best_k_completions(prefix: str):
    ...
    for op_word in first_word(pre_vec[0]):
        ...
        for detail in d[op_word]:
            ...
            if match:
                a = AutoCompleteData.AutoCompleteData(detail[2], detail[0], score, detail[1])
...
def print_top_5(all_matches):
    ...
    if all_matches:
        for auto_comp in all_matches:
            if counter > 5:
                return
            print(str(counter) + ". " + auto_comp.completed_sentence.strip() + " (" + auto_comp.source_text + " " + str(
                auto_comp.offset) + ")" + str(auto_comp.score))
            counter += 1
